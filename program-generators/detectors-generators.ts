    ...
    if (setting === "START") {
        ...
        if (needsFaceOverlay) {
            lines.push(
                ...
                `${generator.INDENT}${generator.INDENT}${generator.INDENT}_h = det.results[0].hypothesis`,
                `${generator.INDENT}${generator.INDENT}${generator.INDENT}_display_face_identity_map[(int(cx), int(cy))] = f"{_h.class_id} ({_h.score:.2f})"`,
                ``,
...
export function get_face_identity(
    ...
    Object.assign(generator.definitions_, {
        ...
        INIT_FACE_REC: [
            ...
            `)`,
            `logging.info("Starting face recognition (subscribed to /vision/face_recognitions)")`,
        ].join("\n"),
    });
